...
bool UImportAssetsCommandlet::ParseParams(const FString& InParams)
{
	...
	FString SourceFolderParam = ParamVals.FindRef(TEXT("sourceFolder"));
	if(!SourcePathParam.IsEmpty())
	{
		GlobalImportData->Filenames.Add(SourcePathParam);
	}
	else if (!SourceFolderParam.IsEmpty())
	{

			if (SourceFolderParam == TEXT("None") || SourceFolderParam == TEXT(""))
			{

				UE_LOG(LogAutomatedImport, Display, TEXT("No Source folder Specified by sourceFolder. Returinng back "));
				//while (1);
				//return 0;
			...
			else
			{
				...
				JoyFileIO_GetFilesInRootAndAllSubFolders
					(Files, SourceFolderParam,
						"fbx"
						//"csv"
						);
				...
				if (Files.Num() > 0)
				{
					...
					for (int i = 0; i < Files.Num(); i++)
					{
						...
						GlobalImportData->Filenames.Add(Files[i]);
						//Files[i] = t.Append(Files[i]);
						//UE_LOG(myCommandlet3Commandlet, Display, TEXT(" %s"), *t);
...
bool UImportAssetsCommandlet::ImportAndSave(const TArray<UAutomatedAssetImportData*>& AssetImportList)
{
	...
	for(const UAutomatedAssetImportData* ImportData : AssetImportList)
	{
		...
		((*((UFbxFactory*)((ImportData)->Factory))).ImportUI)->bImportMaterials = true;
		((*((UFbxFactory*)((ImportData)->Factory))).ImportUI)->bImportTextures = true;
...
bool UImportAssetsCommandlet::JoyFileIO_GetFilesInRootAndAllSubFolders(TArray<FString>& Files, FString RootFolderFullPath, FString Ext)
{
...
bool UImportAssetsCommandlet::JoyFileIO_GetFiles(TArray<FString>& Files, FString sourceFolder, FString Ext)
{
	if (sourceFolder.Len() < 1) return false;
	...
	FString FinalPath = sourceFolder + "/" + Ext;
